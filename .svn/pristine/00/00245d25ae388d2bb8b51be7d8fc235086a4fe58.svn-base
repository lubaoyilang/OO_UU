...
- (NSString *) getTimeDiffString:(NSString*) DateStr
{
    
    NSDate *theDate = [NSDate dateWithString:DateStr];
    
    NSCalendar *gregorianCalendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSGregorianCalendar];
    NSDateComponents *gap = [gregorianCalendar components:NSDayCalendarUnit fromDate:self toDate:theDate options:0];
    
    NSLog(@"DateStr = %@  day = %d  hour = %d   min= %d ",DateStr,ABS(gap.day),ABS(gap.hour),ABS(gap.minute));
...
//
{
    ...
    NSDate *compareDate = [NSDate dateWithString:datestr];
...
+ (NSString*)dateWithcurrentTime:(NSString*)sec
{
    ...
    if (totalSeconds>=60) {
        ...
        return [NSString stringWithFormat:@"%d小时%d分钟",hours, min];
        
    }else{
        if (totalSeconds == 0) {
            return [NSString stringWithFormat:@"1分钟"];
        }
        return [NSString stringWithFormat:@"%d分钟", totalSeconds];
    }
...
- (NSDate *)dateBySetSecond:(NSInteger)second
{
    ...
    NSString *yearStr = [NSString stringWithFormat:@"%d%d", second,second];
...
- (NSDate *)dateBySetMinute:(NSInteger)minute
{
    ...
    NSString *yearStr = [NSString stringWithFormat:@"%d%d", minute,minute];
    dateStr = [dateStr stringByReplacingCharactersInRange:range withString:yearStr];
    NSDate *retDate = [NSDate dateWithString:dateStr];
    return retDate;
}
...
- (NSDate *)dateBySetHour:(NSInteger)hour
{
    ...
    NSString *yearStr = [NSString stringWithFormat:@"%d%d", hour,hour];
    dateStr = [dateStr stringByReplacingCharactersInRange:range withString:yearStr];
    NSDate *retDate = [NSDate dateWithString:dateStr];
    return retDate;
}
...
- (NSDate *)dateByIgnoreOption:(NSDateCompareIgnoreOptions)option
{
    ...
    NSDate *retDate = self;
    retDate = ignoreYear ? [retDate dateBySetYear:REFERENCE_DATE_YEAR] : retDate;
    retDate = ignoreMonth ? [retDate dateBySetMonth:REFERENCE_DATE_MONTH] : retDate;
    retDate = ignoreDay ? [retDate dateBySetDay:REFERENCE_DATE_DAY] : retDate;
    retDate = ignoreHour ? [retDate dateBySetHour:REFERENCE_DATE_HOUR] : retDate;
    retDate = ignoreMinute ? [retDate dateBySetMinute:REFERENCE_DATE_MINUTE] : retDate;
    retDate = ignoreSecond ? [retDate dateBySetSecond:REFERENCE_DATE_SECOND] : retDate;
