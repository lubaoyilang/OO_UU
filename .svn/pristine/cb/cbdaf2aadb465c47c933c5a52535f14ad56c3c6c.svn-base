...
@interface MyOrderListViewController ()<UmpayDelegate>
...
- (void)getUserOrder
{
    ...
    NSString* urlstr = [getUserOrderList stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
- (void)clickInfo:(NSDictionary*)dict
{
    ...
    ConfirmOrderViewController *co = [[ConfirmOrderViewController alloc] init];
    co.productInfo = dict;
    NSLog(@"dict = %@", dict);
...
{
    static NSString *TableSampleIdentifier = @"MyOrderTableViewCell";
    MyOrderTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:TableSampleIdentifier];
    
    
    
    if (cell == nil) {
        
        cell = [[MyOrderTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TableSampleIdentifier cellHeight:CELLH];
        cell.delegate = self;
