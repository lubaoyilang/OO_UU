...
@property (assign,nonatomic)NSInteger preDays;
...
- (void)currentMonthViewDate:(NSArray*)dateArr :(int)days
{
    _selectDateArray = dateArr;
    _preDays = days;
    [frontMonthView checkDate:dateArr :days];
}
...
- (void)swapMonthsAndSlide:(int)direction keepOneRow:(BOOL)keepOneRow
{
    ...
    if (direction == SLIDE_UP) {
        ...
        backMonthView.top = -numWeeksToSlide * 36.0;
    } else {
...
- (void)slide:(int)direction
{
    ...
    [backMonthView  checkDate:_selectDateArray :_preDays];
