...
- (id)initWithFrame:(CGRect)frame delegate:(id<KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic
{
    if ((self = [super initWithFrame:frame])) {
        ...
        UIImageView *back = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
        back.image = [UIImage imageNamed:@"list-bg"];
        [self addSubview:back];
        
        UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0.f, 0.f, frame.size.width, kHeaderHeight)];
        [self addSubviewsToHeaderView:headerView];
        headerView.backgroundColor = RGBCOLOR(235, 234, 231);
...
//        
//        
//        UISwipeGestureRecognizer *resscognizer = [[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(handleSwipeFrom:)];
//        
//        [resscognizer setDirection:(UISwipeGestureRecognizerDirectionLeft)];
//        
//        [self  addGestureRecognizer:resscognizer];
...
- (void)addSubviewsToHeaderView:(UIView *)headerView
{
    ...
    const CGFloat kHeaderVerticalTitle = 13.f;
    ...
    [previousMonthButton setImage:[UIImage imageNamed:@"Kal.bundle/kal_left_arrow_an.png"] forState:UIControlStateHighlighted];
    ...
    [nextMonthButton setImage:[UIImage imageNamed:@"Kal.bundle/kal_right_arrow_an.png"] forState:UIControlStateHighlighted];
    ...
    for (int i = 0; i < 7;  i++) {
        ...
        CGFloat xOffset = [UIScreen mainScreen].bounds.size.width/7;
        CGRect weekdayFrame = CGRectMake(xOffset*i, 40.f, xOffset, kHeaderHeight - 40.f);
        
        UILabel *weekdayLabel = [[UILabel alloc] initWithFrame:weekdayFrame];
        weekdayLabel.backgroundColor = [UIColor colorWithRed:221/255.0 green:221/255.0 blue:221/255.0 alpha:1.0];
        weekdayLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:10];
        weekdayLabel.textAlignment = NSTextAlignmentCenter;
        weekdayLabel.textColor = HexRGB(0x4d4d4d);
        weekdayLabel.text = [weekdayNames objectAtIndex:j];
        [weekdayLabel setAccessibilityLabel:[fullWeekdayNames objectAtIndex:j]];
        [headerView addSubview:weekdayLabel];
        j = (j+1)%7;
    }
    

//    CGRect sss = headerView.bounds;
    // Draw the selected month name centered and at the top of the view
    CGRect monthLabelFrame = CGRectMake((self.width/2.0f) - (kMonthLabelWidth/2.0f),
                                        kHeaderVerticalTitle,
                                        kMonthLabelWidth,
                                        kMonthLabelHeight);
    headerTitleLabel = [[UILabel alloc] initWithFrame:monthLabelFrame];
    headerTitleLabel.backgroundColor = [UIColor clearColor];
    headerTitleLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:15];
    headerTitleLabel.textAlignment = NSTextAlignmentCenter;
    headerTitleLabel.textColor = HexRGB(0x666666);
...
- (void)addSubviewsToContentView:(UIView *)contentView
{
    ...
    CGRect fullWidthAutomaticLayoutFrame = CGRectMake(0.f, 0.f, [UIScreen mainScreen].bounds.size.width, 0.f);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (object == self.gridView && [keyPath isEqualToString:@"frame"]) {
        ...
        if (_delegate && [(id)_delegate respondsToSelector:@selector(changeFrame:)]) {
            [_delegate changeFrame:frame];
